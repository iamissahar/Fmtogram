...
func TestAnswerInlineQuery(t *testing.T) {
	...
	if err = tc.inline.WritePhoto(&types.InlineQueryResultPhoto{Type: "photo", PhotoURL: photodata[tc.token][2]}); err != nil {
		t.Fatal(err)
	}
	...
	if err = tc.inline.WriteButton(&types.InlineQueryResultsButton{Text: "YOUTUBE", WebApp: &types.WebAppInfo{Url: "https://youtube.com"}}); err != nil {
		t.Fatal(err)
	}
...
func TestAnswerShippingQuery(t *testing.T) {
	...
	// if err = tc.pay.WriteShippingOptions([]*types.ShippingOption{{ID: "VALID_ID", Title: invtitle, Prices: prices}}); err != nil {
	// 	t.Fatal(err)
	// }
...
func TestSetPassport(t *testing.T) {
	...
	if err = tc.prm.WriteErrors([]*types.PassportElementError{{
		Source:    "data",
