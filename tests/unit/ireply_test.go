...
type rpTestContainer struct {
	...
	inputArr      [][]int
	isExpectedErr []bool
	firststep     func(_ fmtogram.IReply, plan []int) error
	setplan       [][]int
	codeErr       []string
	amount, until int
	buildF        func(rptc rpTestContainer, rp fmtogram.IReply, i int) *replyT
}

func putReplyNewButton(rptc rpTestContainer, rp fmtogram.IReply, i int) *replyT {
	return &replyT{rptc.name, "", rptc.inputInt[i], nil, rp.NewButton, rptc.isExpectedErr[i], rptc.codeErr[i]}
}

func putReplySet(rptc rpTestContainer, rp fmtogram.IReply, i int) *replyT {
	return &replyT{rptc.name, "", [2]int{}, rptc.inputArr[i], rp.Set, rptc.isExpectedErr[i], rptc.codeErr[i]}
}

func putReplyWriteIsPersistent(rptc rpTestContainer, rp fmtogram.IReply, i int) *replyT {
	return &replyT{rptc.name, "", [2]int{}, nil, rp.WriteIsPersistent, rptc.isExpectedErr[i], rptc.codeErr[i]}
}

func putReplyWriteResizeKeyboard(rptc rpTestContainer, rp fmtogram.IReply, i int) *replyT {
	return &replyT{rptc.name, "", [2]int{}, nil, rp.WriteResizeKeyboard, rptc.isExpectedErr[i], rptc.codeErr[i]}
}

func putReplyWriteInputFieldPlaceholder(rptc rpTestContainer, rp fmtogram.IReply, i int) *replyT {
	return &replyT{rptc.name, rptc.inputStr[i], [2]int{}, nil, rp.WriteInputFieldPlaceholder, rptc.isExpectedErr[i], rptc.codeErr[i]}
}

func putReplyWriteRemove(rptc rpTestContainer, rp fmtogram.IReply, i int) *replyT {
...
func (rptc *rpTestContainer) newButton() {
	...
	rptc.firststep = fmtogram.IReply.Set
...
func (rp *replyT) callDIntF(f func(int, int) (fmtogram.IReplyButton, error), t *testing.T) {
	if !rp.isExpectedErr {
...
func (rp *replyT) startTest(part string, i int, t *testing.T) {
	...
	case func(string) error:
		...
		rp.callStrF(f, t)
	case func(int, int) (fmtogram.IReplyButton, error):
...
func (rptc *rpTestContainer) createTestArrays(msg *fmtogram.Message, t *testing.T) ([]UnitTest, []UnitTest) {
	var rp fmtogram.IReply
	var err error
	a, b := make([]UnitTest, rptc.until), make([]UnitTest, rptc.amount-rptc.until)
	for i, j := 0, 0; i < rptc.amount; i++ {
		if i < rptc.until {
			if rp, err = fmtogram.NewKeyboard().WriteReply(); err != nil {
				t.Fatal(err)
			}
...
func (rptc *rpTestContainer) mainLogic(msg *fmtogram.Message, t *testing.T) {
